...
import { Alert, Badge, Button, Card, Form, Modal, Spinner, Accordion, Table } from "react-bootstrap";
import { useNavigate, useParams } from "react-router-dom";
import { getPrivateLeagueDetail } from "../api/privateLeagueDetail";
import { previewPrivateLeagueSchedule, publishPrivateLeagueSchedule } from "../api/privateLeagueSchedule";
...
export default function MyLeagueSchedulePage() {
  ...
  const titleType = useMemo(() => (type === "double" ? "Ida y vuelta" : "Solo ida"), [type]);
  ...
  const loadLeague = async () => {
    ...
    try {
      ...
      if (status === 404) setErrorLeague("No tienes acceso a esta liga (o no existe).");
      else if (status === 401) setErrorLeague("Necesitas iniciar sesión.");
      else setErrorLeague("No se pudo cargar la liga.");
    } finally {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const status = e?.response?.status;
      if (status === 403) setErrorPreview("No tienes permisos para generar el calendario (solo admin/owner).");
      else if (status === 404) setErrorPreview("No tienes acceso a esta liga (o no existe).");
      else if (status === 422) setErrorPreview("No se pudo generar: revisa que haya equipos suficientes.");
      else setErrorPreview("No se pudo generar el preview del calendario.");
    } finally {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Redirige a jornadas (FE-07)
      navigate(`/mis-ligas/${leagueId}/jornadas`);
    } catch (e) {
      const status = e?.response?.status;
      if (status === 409) setPublishError("El calendario ya está publicado y no se puede regenerar.");
      else if (status === 403) setPublishError("No tienes permisos para publicar el calendario.");
      else if (status === 422) setPublishError("No se pudo publicar: revisa que haya equipos suficientes.");
      else setPublishError("No se pudo publicar el calendario.");
    } finally {
      setPublishing(false);
    }
  };
      ...
      <div className="d-flex justify-content-center">
        <Card className="shadow-sm border-0" style={{ maxWidth: 900, width: "100%", borderRadius: "1rem" }}>
          <Card.Body className="p-4">
            <Alert variant="danger" className="mb-3">{errorLeague}</Alert>
            <Button variant="secondary" onClick={() => navigate("/mis-ligas")}>Volver</Button>
          </Card.Body>
        </Card>
      </div>
    );
  ...
  // Guardia UI (el BE ya lo protege, pero aquí damos feedback claro)
  if (!canManage) {
    return (
      <div className="d-flex justify-content-center">
        <Card className="shadow-sm border-0" style={{ maxWidth: 900, width: "100%", borderRadius: "1rem" }}>
          <Card.Body className="p-4">
            <Alert variant="warning" className="mb-3">
              No tienes permisos para generar/publicar el calendario (solo admin/owner).<br />
              Tu rol: <strong>{role}</strong>
            </Alert>
            <Button variant="secondary" onClick={() => navigate(`/mis-ligas/${leagueId}`)}>Volver a la liga</Button>
          </Card.Body>
        </Card>
      </div>
    );
  ...
  return (
    <div className="d-flex justify-content-center">
      <Card className="shadow-sm border-0" style={{ maxWidth: 1000, width: "100%", borderRadius: "1rem" }}>
        ...
        >
          <div className="d-flex justify-content-between align-items-start gap-3">
            ...
            <div className="d-flex flex-column align-items-end gap-2">
              <Badge bg="light" text="dark">Admin/Owner</Badge>
              <Button size="sm" variant="light" onClick={() => navigate(`/mis-ligas/${leagueId}`)}>
                Volver
              </Button>
            </div>
          </div>
        ...
        <Card.Body className="p-4">
          ...
          <div className="d-flex flex-wrap gap-2 mb-3">
            <Badge bg="secondary">Equipos: {Array.isArray(league?.teams) ? league.teams.length : 0}</Badge>
            <Badge bg="secondary">Formato: {titleType}</Badge>
          </div>
          ...
          {errorPreview && <Alert variant="danger" className="mt-3">{errorPreview}</Alert>}
                      ...
                      <Table responsive bordered hover className="mb-0 align-middle">
                        <thead>
                          <tr>
                            ...
                            <th style={{ width: "10%" }} className="text-center">VS</th>
      ...
      <Modal show={showConfirm} onHide={() => setShowConfirm(false)} centered>
        ...
        <Modal.Body>
          <p className="mb-2">
            Vas a publicar el calendario en formato <strong>{titleType}</strong>.
          </p>
          <p className="mb-0 text-muted">
            Una vez publicado, no se podrá regenerar (según reglas de la liga).
          </p>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowConfirm(false)} disabled={publishing}>
            Cancelar
          </Button>
